...
| `chronicle_search` | Fork-local: semantic search over the archived conversation chronicle (`sylva_chronicle`) by topic, speaker, or date range |

> **Fork note (Sylva / PRD-029):** per-turn passive fact extraction is disabled
> in this fork. `sync_turn()` performs no writes — durable memory is written only
> via explicit `mem0_add` tool calls and the governed consolidation pipeline.
...
- `mem0_add` stores verbatim (no extraction). (In this fork, per-turn `sync_turn`
  extraction is intentionally disabled — see the fork note above.)
